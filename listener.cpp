...
void export_to_json_unsafe() {
    ...
    int start = (security_logs.size() > 10) ? security_logs.size() - 10 : 0;
    bool first_log = true;
    for (int i = start; i < security_logs.size(); i++) {
        if (!first_log) file << ",\n";
...
void log_event_unsafe(const std::string& message) {
    ...
    // Affichage terminal minimaliste
...
std::string rsa_decrypt(EVP_PKEY* priv_key, const unsigned char* encrypted_data, size_t data_len) {
    ...
    EVP_PKEY_decrypt_init(ctx); EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PADDING);
    size_t outlen; EVP_PKEY_decrypt(ctx, NULL, &outlen, encrypted_data, data_len);
    unsigned char* out = new unsigned char[outlen];
    if (EVP_PKEY_decrypt(ctx, out, &outlen, encrypted_data, data_len) <= 0) { delete[] out; EVP_PKEY_CTX_free(ctx); return ""; }
    std::string result((char*)out, outlen); delete[] out; EVP_PKEY_CTX_free(ctx); return result;
}

EVP_PKEY* generate_rsa_key() {
    EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, NULL); EVP_PKEY_keygen_init(ctx);
    EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, 2048); EVP_PKEY *pkey = NULL; EVP_PKEY_keygen(ctx, &pkey);
...
std::string get_public_key_pem(EVP_PKEY* pkey) {
    ...
    char *data; long len = BIO_get_mem_data(bio, &data); std::string result(data, len);
...
std::string decrypt_aes256(const unsigned char* ciphertext, int ciphertext_len, const unsigned char* key, const unsigned char* iv) {
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new(); unsigned char plaintext[2048] = {0}; int len = 0, plaintext_len = 0;
    EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, key, iv); EVP_DecryptUpdate(ctx, plaintext, &len, ciphertext, ciphertext_len); plaintext_len = len;
    if (EVP_DecryptFinal_ex(ctx, plaintext + len, &len) != 1) { EVP_CIPHER_CTX_free(ctx); return "ERREUR"; }
    plaintext_len += len; EVP_CIPHER_CTX_free(ctx); return std::string((char*)plaintext, plaintext_len);
}

std::string encrypt_aes256(const std::string& plaintext, const unsigned char* key, const unsigned char* iv) {
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new(); unsigned char ciphertext[2048] = {0}; int len = 0, ciphertext_len = 0;
    EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, key, iv); EVP_EncryptUpdate(ctx, ciphertext, &len, (const unsigned char*)plaintext.c_str(), plaintext.length()); ciphertext_len = len;
    EVP_EncryptFinal_ex(ctx, ciphertext + len, &len); ciphertext_len += len;
    EVP_CIPHER_CTX_free(ctx); return std::string((char*)ciphertext, ciphertext_len);
}
...
void handle_client(int client_socket, struct sockaddr_in client_addr) {
    ...
    if (rsa_bytes < 256) { std::lock_guard<std::mutex> lock(system_mutex); active_nodes.erase(client_socket); close(client_socket); return; }
    ...
    memcpy(this_node.session_key, decrypted_session.c_str(), 32); memcpy(this_node.session_iv, decrypted_session.c_str() + 32, 16);
    ...
    while (true) {
        ...
        try {
            size_t id_pos = decrypted.find("\"ID\":\""); if (id_pos != std::string::npos) this_node.id = decrypted.substr(id_pos + 6, decrypted.find("\"", id_pos + 6) - (id_pos + 6));
            size_t host_pos = decrypted.find("\"HOST\":\""); if (host_pos != std::string::npos) this_node.hostname = decrypted.substr(host_pos + 8, decrypted.find("\"", host_pos + 8) - (host_pos + 8));
            size_t ram_pos = decrypted.find("\"RAM_MB\":"); if (ram_pos != std::string::npos) this_node.ram_mb = std::stol(decrypted.substr(ram_pos + 9, decrypted.find(",", ram_pos) - (ram_pos + 9)));
            size_t procs_pos = decrypted.find("\"PROCS\":"); if (procs_pos != std::string::npos) this_node.procs = std::stoi(decrypted.substr(procs_pos + 8, decrypted.find("}", procs_pos) - (procs_pos + 8)));
        } catch (...) {}
        ...
        this_node.latency = diff; std::string command_to_send;

        {
            ...
            if (diff < 800) {
                ...
                this_node.status = "STABLE"; command_to_send = "CMD:STANDBY";
                active_nodes[client_socket] = this_node;
            }
            export_to_json_unsafe();
        }
...
int main() {
    ...
    while (true) {
        ...
        if (new_socket >= 0) std::thread(handle_client, new_socket, client_addr).detach();
    }
    return 0;
}
