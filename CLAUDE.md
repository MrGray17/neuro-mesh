...
```bash
# Build everything (eBPF skeleton + neuro_agent + simulate_threat + test_crypto)
make clean && make
...
# Threat simulation (injects PBFT consensus targeting NODE_5)
./bin/simulate_threat
...
python3 orchestration/c2_server.py
python3 orchestration/brain_ai.py
...
Neuro-Mesh is a distributed P2P security mesh. C++ sovereign nodes use eBPF to detect kernel-level anomalies, run a PBFT consensus protocol over UDP to verify threats, then execute network isolation against compromised peers.

### Data Flow

```
eBPF kernel probe (kernel/sensor.bpf.c)
  → SovereignCell (cell/) polls ring buffer, feeds InferenceEngine
    → InferenceEngine (cell/) entropy analysis
      → MeshNode (consensus/) UDP broadcast PBFT voting
        → PBFTConsensus (consensus/PBFT.hpp) multi-hop state machine
          → SystemJailer (jailer/) iptables isolation
            → AuditLogger (telemetry/) UDP telemetry to C2
```
...
| `cell/` | Sovereign intelligence | `SovereignCell.hpp/.cpp` (agent core), `InferenceEngine.hpp/.cpp` (entropy anomaly detection) |
...
| `jailer/` | Isolation execution | `SystemJailer.hpp/.cpp` (iptables + eBPF blocklist + process imprisonment) |
| `telemetry/` | Structured logging | `AuditLogger.hpp/.cpp` (UDP JSON), `TelemetryExporter.hpp` (POSIX-locked file writes) |
| `orchestration/` | Python control plane | `c2_server.py`, `brain_ai.py`, `bridge_api.py`, `web_server.py`, `neuro_ctl.py`, `mesh_manager.py` |
| `tools/` | Test/sim utilities | `simulate_threat.cpp`, `test_crypto.cpp` |
| `dashboard-react/` | React SOC dashboard | KPI cards, topology graph, agent table, event logs, time-series charts |
| `main.cpp` | Entry point | Initializes SystemJailer + MeshNode, idles on signal loop |
...
- **Safe list** — `SystemJailer::add_safe_node()` prevents a node from ever isolating itself or critical infrastructure, even if PBFT consensus demands it.
...
- **fork+exec iptables** — SystemJailer uses `fork()` + `execv()` to call iptables with arguments as separate `argv` entries, eliminating shell injection vectors.
...
- **Continuous eBPF drain** — `SovereignCell::telemetry_loop()` drains the ring buffer in a tight `while(ring_buffer__poll()>0)` loop before analysis, preventing kernel-side event loss.
- **IPC socket** — `main.cpp` creates a Unix domain socket at `/tmp/neuro_mesh_{id}.sock` for C2 command delivery (ISOLATE, VACCINATE, SHUTDOWN).
