...
Each C++ node runs its own TelemetryBridge WebSocket on ports 9000-9004.
...
LISTEN_PORT = 9001
# Docker bridge gateway — host-network containers are reachable at this IP.
# On Docker Desktop (WSL2/Windows/Mac), 127.0.0.1 also works.
# On native Linux, use the bridge gateway (typically 172.17.0.1 or 172.18.0.1).
HOST_IP = "172.18.0.1"

BACKENDS = [
    f"ws://{HOST_IP}:9000",   # ALPHA
    f"ws://{HOST_IP}:9010",   # BRAVO
    f"ws://{HOST_IP}:9020",   # CHARLIE
    f"ws://{HOST_IP}:9030",   # DELTA
    f"ws://{HOST_IP}:9040",   # ECHO
]
...
async def proxy(peer_sock, path="/"):
    ...
    if backend is None:
        print(f"[WS-PROXY] No backend available — rejecting client {peer_addr}", flush=True)
...
async def main():
    ...
    print(f"[WS-PROXY] Listening on 0.0.0.0:{LISTEN_PORT} → backends: {backends_str}", flush=True)
